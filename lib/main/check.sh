...
build_validate_buildOnly() {
	...
	local _all_valid_in_chroot="$(list_of_building_all_valid_in_chroot)"
	local _build_in_chroot
	local _build_default="$(default_task_list)"
	local _all_valid_buildOnly="$(default_task_list),$(list_of_bsp_desktop_packages),chroot,$(list_of_building_all_valid_in_chroot)"
	...
	# collective target = "chroot"
	if string_is_contain "$_buildOnly" "chroot"
	then
	...
	for _taskName in ${_buildOnly//,/ }; do
		...
		if string_is_contain "${_all_valid_in_chroot}" "$_taskName"
		then
			_build_in_chroot+="${_taskName} "
		fi
	done
